...
const Dropzone = ({ className = '', onDrop, disabled = false, maxFiles, maxSize = 0, accept }: DropzoneProps) => {
  ...
  let dropzoneStylesNew = 'w-full text-center bg-white dark:bg-zinc-900 rounded-2xl';

  if ( state === 'over' && !disabled ) {
    dropzoneStylesNew = `${dropzoneStylesNew} relative shadow-blue-400/60 before:content-["_"] before:block before:w-full before:h-full before:bg-blue-500 before:blur-lg dark:before:blur-xl before:opacity-60 before:absolute before:top-0 before:right-0 before:z-[-1] before:animate-faderight before:bg-[length:400%] before:bg-gradient-to-r before:from-blue-500 dark:before:from-blue-400 before:from-[10%] before:via-purple-600 dark:before:via-purple-500 before:to-blue-500 dark:before:to-blue-400 before:to-[90%]`;
  } else if ( disabled ) {
    dropzoneStylesNew = `${dropzoneStylesNew} shadow-[0px_4px_14px_0px_rgba(0,0,0,0.15)] shadow-red-200 dark:shadow-zinc-900`;
  } else {
    dropzoneStylesNew = `${dropzoneStylesNew} shadow-[0px_4px_14px_0px_rgba(0,0,0,0.15)] shadow-zinc-200 dark:shadow-zinc-900`;
  }
  ...
  const handleOnDropNew = useCallback((acceptedFiles: Array<File>, rejectedFiles: Array<FileRejection>, event: DropEvent) => {
            ...
            function onFileLoad() {
              ...
              if (typeof result === 'string') {
                const upload: UserUpload = {
                  ...
                  size: file.size,
                  file,
                  status: isAcceptedFileType(file.type) ? 'accepted' : 'rejected'
                };
  ...
  return (
    <>
      <div className={cn(dropzoneStylesNew, className)}>
        <div
          ref={ref}
